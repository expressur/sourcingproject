...
ini_set('display_errors', 1);
	...
	if (!empty($_POST)) { // Si tous les input sont remplis
		...
		if (empty($_POST["Mail_Utilisateur"])) $errors['Mail_Utilisateur'] = "Identifiant : Aucun identifiant n'a été entré"; // On vérifie si l'identifiant a été entré
		else if (empty($_POST["Mdp_Utilisateur"]))  $errors['Mdp_Utilisateur'] = "Mot de passe : Aucun mot de passe n'a été entré"; // On vérifie si le mot de passe a été entré
		else {
			...
			if (!$checked_login) $errors["Mail_Utilisateur"] = "Identifiant : Cet identifiant est incorrect"; // On affiche un message d'erreur
				...
				if ($verify_password == 0) $errors["Mdp_Utilisateur"] = "Mot de passe : Le mot de passe est incorrect";	
			}
		}
		echo $user_login ;
